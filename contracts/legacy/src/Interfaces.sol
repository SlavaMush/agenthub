...
interface IERC8004Reputation {
    ...
    function getReview(address agent, uint256 index)
        external
        view
        returns (
            address reviewer,
...
interface IMemoryNFT {
    function modules(uint256 tokenId)
        external
        view
        returns (
            string memory cid,
...
interface IServiceListing {
    function getListing(uint256 listingId)
        external
        view
        returns (address seller, address winner, uint256 escrowAmount, bool disputed, bool delivered);
    function FEE_BPS() external view returns (uint256);
